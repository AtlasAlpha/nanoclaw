...
registerProviderContainerConfig('opencode', (ctx) => {
  ...
  // Load auth token from process.env or .env so OpenCode can authenticate with Zen/Go
  const authToken = ctx.hostEnv.OPENCODE_AUTH_TOKEN || readEnvFile(['OPENCODE_AUTH_TOKEN']).OPENCODE_AUTH_TOKEN;
  if (authToken) env['OPENCODE_AUTH_TOKEN'] = authToken;
  // Forward any provider-specific API key (e.g. ANTHROPIC_API_KEY for direct Anthropic)

...
export function getMessagingGroup(id: string): MessagingGroup | undefined {
  return queryOne<MessagingGroup>(getDb(), 'SELECT * FROM messaging_groups WHERE id = ?', [id]);
}

export function getMessagingGroupByPlatform(channelType: string, platformId: string): MessagingGroup | undefined {
  return queryOne<MessagingGroup>(
    getDb(),
...
export function getMessagingGroupWithAgentCount(
  ...
  const row = queryOne<MessagingGroup & { agent_count: number }>(
    getDb(),
...
export function getAllMessagingGroups(): MessagingGroup[] {
  return queryAll<MessagingGroup>(getDb(), 'SELECT * FROM messaging_groups ORDER BY name');
}

export function getMessagingGroupsByChannel(channelType: string): MessagingGroup[] {
  return queryAll<MessagingGroup>(getDb(), 'SELECT * FROM messaging_groups WHERE channel_type = ?', [channelType]);
}
...
export function updateMessagingGroup(
  ...
  params.push(id);
  getDb().run(`UPDATE messaging_groups SET ${fields.join(', ')} WHERE id = ?`, params as never);
}

export function deleteMessagingGroup(id: string): void {
  getDb().run('DELETE FROM messaging_groups WHERE id = ?', [id]);
}
...
export function setMessagingGroupDeniedAt(id: string, deniedAt: string | null): void {
  getDb().run('UPDATE messaging_groups SET denied_at = ? WHERE id = ?', [deniedAt, id]);
}
...
export function getMessagingGroupAgents(messagingGroupId: string): MessagingGroupAgent[] {
  return queryAll<MessagingGroupAgent>(
    getDb(),
...
export function getMessagingGroupAgentByPair(
  ...
  return queryOne<MessagingGroupAgent>(
    getDb(),
...
export function getMessagingGroupAgent(id: string): MessagingGroupAgent | undefined {
  return queryOne<MessagingGroupAgent>(getDb(), 'SELECT * FROM messaging_group_agents WHERE id = ?', [id]);
}
...
export function updateMessagingGroupAgent(
  ...
  params.push(id);
  getDb().run(`UPDATE messaging_group_agents SET ${fields.join(', ')} WHERE id = ?`, params as never);
}

export function deleteMessagingGroupAgent(id: string): void {
  getDb().run('DELETE FROM messaging_group_agents WHERE id = ?', [id]);
}
...
export function getMessagingGroupsByAgentGroup(agentGroupId: string): MessagingGroup[] {
  return queryAll<MessagingGroup>(
    getDb(),

...
registerResource({
  ...
  description: 'Agent destination — per-agent routing entry and ACL. Each row authorizes an agent to send messages to a target (channel or another agent) and assigns a local name the agent uses to address it.',
  idColumn: 'agent_group_id',
  columns: [
    { name: 'agent_group_id', type: 'string', description: 'The agent that owns this destination.' },
    { name: 'local_name', type: 'string', description: 'Name the agent uses to address this target. Unique per agent.' },
    { name: 'target_type', type: 'string', description: '"channel" for messaging group targets, "agent" for agent-to-agent.', enum: ['channel', 'agent'] },
  ...
  customOperations: {
    add: {
      ...
      handler: async (args) => {
        ...
        if (!targetType || !['channel', 'agent'].includes(targetType)) throw new Error('--target-type must be channel or agent');
        if (!targetId) throw new Error('--target-id is required');
        getDb().run("INSERT INTO agent_destinations (agent_group_id, local_name, target_type, target_id, created_at) VALUES (?, ?, ?, ?, datetime('now'))", [agentGroupId, localName, targetType, targetId] as never);
    ...
    remove: {
      ...
      handler: async (args) => {
        ...
        if (!agentGroupId) throw new Error('--agent-group-id is required');
        if (!localName) throw new Error('--local-name is required');
        const result = run(getDb(), 'DELETE FROM agent_destinations WHERE agent_group_id = ? AND local_name = ?', [agentGroupId, localName]);

...
registerResource({
  ...
  plural: 'roles',
  table: 'user_roles',
  description: 'Role grant on a user. Owner is always global. Admin can be global or scoped to an agent group. Admin @ A implies membership in A.',
  idColumn: 'user_id',
  columns: [
    { name: 'user_id', type: 'string', description: 'The user receiving the role.' },
    { name: 'role', type: 'string', description: 'Owner (global) or admin (global or scoped).', enum: ['owner', 'admin'] },
  ...
  customOperations: {
    grant: {
      ...
      handler: async (args) => {
        ...
        getDb().run('INSERT OR IGNORE INTO user_roles (user_id, role, agent_group_id, granted_at) VALUES (?, ?, ?, datetime(\'now\'))', [userId, role, groupId] as never);
